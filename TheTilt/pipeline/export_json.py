...
def warn_uncovered_countries(player_tilt: pd.DataFrame) -> list:
    """Report players in the data with no entry in config/player_countries.yaml.

    The yaml is a *committed* config regenerated by hand
    (`build_player_countries.py`), while this exporter runs twice daily on
    freshly downloaded data. So every player who debuts between regenerations
    silently falls through `country_for` to `default_country: IN` and is
    painted with an India flag site-wide.

    That is what actually happened to Ben Dwarshuis (PBKS 2025), the third
    case in issue #241: Wikidata had him right as Australian all along, he was
    just absent from a stale yaml. Nothing in the pipeline said so, because
    the fallback is silent by design. This makes it loud in the refresh log.
    """
    cache = load_player_countries()
    if "player_id" not in player_tilt.columns:
        return []
    known = cache["_map"]
    missing = [
        (str(r["player_id"]), str(r["player"]))
        for _, r in player_tilt.iterrows()
        if r.get("player_id") and str(r["player_id"]) not in known
    ]
    if missing:
        print(f"\n  WARNING: {len(missing)} player(s) missing from config/player_countries.yaml — "
              f"defaulting to {cache['_default']} flag:")
        for pid, name in missing:
            print(f"    {pid}  {name}")
        print("    Re-run: ./venv/bin/python pipeline/download_people.py && "
              "./venv/bin/python pipeline/build_player_countries.py --allow-default-in")
    return missing
    ...
    for _, row in qualified.iterrows():
        ...
        for season in sorted(all_seasons, key=str):
            ...
            for mid in all_match_ids:
                ...
                # 2 dp, matching every other batting-SR export site (career/season
                # batting_stats, season leaders, team-season roster) — the match
                # log used to be 1 dp, so player.html rendered the same quantity
                # at two precisions on one page (#245).
                bat_sr_val = round(bat_runs_val / max(bat_balls_val, 1) * 100, 2) if bat_runs_val is not None else None
        ...
        for inn_num in sorted(mdf["innings"].unique()):
            ...
            for _, r in dismissals_df.iterrows():
                ...
                if key is not None:
                    dismissal_map[key] = {
                        ...
                        "over": over_label,
                        # Identity + timing, so a dismissed player who never got
                        # on strike can still be given a card row below (#239).
            ...
            for _, r in bat_card.iterrows():
                ...
                batting_scorecard.append({
                    ...
                    "runs": int(r["runs"]),
                    "balls": int(r["balls"]),
                    "sr": round(r["runs"] / max(r["balls"], 1) * 100, 2),  # 2 dp everywhere (#245)
                    "tilt": round(r["tilt"], 5),
                    ...
                    "not_out": d is None,
                    "_order": int(r["first_ball"]),
                })

            # A batter dismissed without ever facing a legal delivery — run out
            # backing up at the non-striker's end, or run out off a wide — never
            # appears in the `batter` column, so the groupby above emits no row
            # for them. They still fall, so the card showed one fewer dismissed
            # batter than the innings wicket count, and the fall-of-wickets line
            # underneath named someone with no row above it (issue #239).
            #
            # Distinct from #194, which was a *missing dismissal*; here the
            # dismissal is recorded correctly and only the batting row is
            # absent. Give them a 0 (0) row, ordered by the delivery they were
            # dismissed on — they were at the crease for it, so it slots them
            # into the batting order between the batter who came in before them
            # and the one who came in after.
            for key, d in dismissal_map.items():
                if key in carded_keys:
                    continue
                did = d["player_id"]
                batting_scorecard.append({
                    "player": d["player"],
                    "slug": slug_lookup.get(did, ""),
                    "country": country_for(did),
                    "runs": 0,
                    "balls": 0,
                    "sr": 0.0,
                    # No ball was ever credited to them, so they moved the win
                    # probability by nothing — the delta on the dismissal
                    # delivery belongs to the striker. Keeping this 0.0 also
                    # leaves the innings TILT total (a sum over these rows)
                    # exactly where it was.
                    "tilt": 0.0,
                    "dismissal_kind": d["kind"],
                    "dismissal_bowler": d["bowler"],
                    "dismissal_fielders": d["fielders"],
                    "fall_of_wicket_score": d["score_at_fall"],
                    "fall_of_wicket_over": d["over"],
                    "wicket_position": d["wicket_position"],
                    "not_out": False,
                    "_order": d["ball_number"],
                })
            batting_scorecard.sort(key=lambda b: b["_order"])
...
def export_team_details(
    ...
    for canonical, slug in TEAM_SLUG.items():
        ...
        team_doc = {
            ...
            "career": {
                # matches / win_pct are NR-inclusive, matching the season rows
                # rendered directly below them on the same page (issue #240).
                "matches": int(tr["matches"]),
                "wins": int(tr["wins"]),
                "losses": int(tr["losses"]),
                "no_results": int(tr.get("no_results", 0)),
...
def export_all(
    ...
    warn_uncovered_countries(player_tilt)
