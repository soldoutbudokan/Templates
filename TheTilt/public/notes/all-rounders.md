...
Note the split personalities in that table. JR Hopes is the most extreme: a 20-match sample (one of them a DLS-shortened match whose re-scoring moved his career average materially) leaves his raw rank at #<span id="ar-hopes-raw-prose">161</span> while the confidence floor — which prices in how *erratic* a sample is, not just how long — drops him to #<span id="ar-hopes-floor-prose">339</span>. Stokes is the mirror image of the veterans: 45 matches, a respectable raw #<span id="ar-stokes-raw-prose">116</span>, but a floor near #<span id="ar-stokes-floor-prose">209</span> because the model doesn't trust the small sample. Sample size, not just role, drives where an all-rounder lands.
