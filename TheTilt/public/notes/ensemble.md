...
The disagreement statistic is the real story: the K members routinely disagree by a few pp on the same ball, with worst-case ~10pp — and the spread widened when per-member early stopping moved onto per-member train-carved folds (#193), which is added member diversity, not added ensemble noise. Each individual training run is essentially sampling from this distribution. The ensemble averages it out.
