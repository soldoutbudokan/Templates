...
- **Venue deduplication.** Cricsheet ships 60 raw venue strings — a mix of stadium-only and stadium-with-city ("Wankhede Stadium" vs "Wankhede Stadium, Mumbai") and renamings ("Feroz Shah Kotla" → "Arun Jaitley Stadium", "Sardar Patel Stadium, Motera" → "Narendra Modi Stadium, Ahmedabad"). After mapping, the model sees **37 canonical venues**, which materially reduces categorical overfitting.
...
| `balls_remaining` | 1 — 120 | Deliveries left in the innings (counts every delivery incl. wides/no-balls; legal-only counting is deferred — issue #192) |
...
| Metric | Value | Notes |
|:--|:--|:--|
| **Brier score** | 0.192 | Mean squared error between predicted prob and outcome. 0 is perfect; 0.25 is "always predict 0.5". |
| **Log loss** | 0.559 | Cross-entropy. Lower is better. |
| **AUC** | 0.773 | Discrimination — given a ball with a winner-side and loser-side, the model orders them correctly 77% of the time. |
...
| **K-member disagreement (std)** | median 3.6pp · p95 6.8pp · max 10.6pp | How much the 100 ensemble members disagree on a typical holdout ball. The ensemble's predict_proba averages this away; individual members live in this distribution. |
...
- **Era effects exist but are modest.** `season_numeric` shows up at 146 — enough to keep modern hitters from being treated as identical to 2010 hitters, not so much that it dominates.
...
`k` is the **within-player variance ÷ between-player variance**, estimated empirically from match-level TILT distributions. For IPL data, **k ≈ 5.2** (re-estimated on every pipeline run).
...
Win probability swings are **structurally larger in the 2nd innings**, because the target is known and every ball directly resolves the chase math. On average, a 2nd-innings ball produces a `|delta_wp|` that is **1.58×** a 1st-innings ball. The gap widens through the innings — **1.25× in the powerplay, 1.67× in the middle overs, and 2.14× in the death (overs 16–20)**.

The downstream effect on single-match GOAT-type rankings is large: 100% of the top-50 batting GOAT performances and 96% of the top-50 bowling GOAT performances are from 2nd innings. The GOAT page therefore exposes innings-filtered views so within-innings comparisons are fair.
...
Top 10 players by **TILT floor** (90% CI lower bound), rendered live from the current [rankings](rankings.html). *As of <span id="top10-asof">…</span> (post K=100 ensemble, post Step-3 removal, post `recent_wickets` rolling-window fix).* During an in-progress season the data refreshes through the day, so this list reshuffles intra-tournament — especially around the medium- and low-confidence ranks, where a handful of matches moves the order.

<div id="top10-floor" class="loading">loading…</div>
