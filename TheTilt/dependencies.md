...
| `public/notes/last-ball-snap.md` | Pre-fix model averages (`wp_after` 0.928 on winning chases, 0.161 on losing) are historical — frozen as the diagnostic that drove `compute_tilt.apply_match_terminal_snap`. Top-10 by total TILT after-table embeds live names (DJ Bravo #62→#10 the headline). Biggest gainers/losers tables (50+ matches) embed live ranks for Vinay Kumar, Avesh, HV Patel, Bravo, Russell on one side; Uthappa, Dhoni, Miller, AR Patel on the other. Tied-match count (16) is also embedded. Re-pull after-tables from `tilt_rankings.json` if the snap logic changes. |
