...
- The `Players ranked (≥ 10 matches)` figure (472 as of 2026-05-30) in about.md and README will change. **This also drifts on every twice-daily data refresh** — not just on threshold changes — because the ongoing season tips new debutants across the ≥10-match line. Re-verify it against the count of entries in `tilt_rankings.json` on any data-refresh commit.
- Refresh those.
...
|:--|:--|
| `public/about.md` | Match/ball counts, hyperparameters, Brier/AUC/calibration, feature importances, k value, innings asymmetry ratios, innings-boundary recalibration numbers (median \|jump\|, mean signed jump, season trend). *(§11's top-10 table + "as of" date are no longer here — they're live-rendered client-side by `about.html` from `tilt_rankings.json`/`meta.json`, so they can't go stale. Only §11's qualitative prose is hand-maintained.)* |
